...
export function getAlertmanagerAbilities({
  ...
  const canViewSilence = isGrafanaFlavoredAlertmanager
    ? ctx.hasPermission(instancePermissions.read) || ctx.hasPermission(AccessControlAction.AlertingSilenceRead)
    : ctx.hasPermission(instancePermissions.read);
